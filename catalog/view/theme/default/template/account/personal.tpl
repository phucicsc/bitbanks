<?php echo $header; ?>
<?php echo $column_left; ?>

<div class="container">
    <div id="wrapper">
        <div id="layout-static">
            <div class="static-content-wrapper">
                <div class="static-content">
                    <div class="page-content">
                        <ol class="breadcrumb">
                            <li>
                                <a href="#">Home</a>
                            </li>
                            <li style="padding:0">
                                >
                            </li>
                            <li class="active">
                                <a href="javascript:void(0)">Downline Tree</a>
                            </li>
                        </ol>
                        <div class="container-fluid" style="margin-top:0px !important">
                            <div id="content">
                                <div class="personal-left">
                                   <!--  Total floor: <span style="font-weight: bold;" id="ajax_floor"><?php echo $floor;?></span>
                                    | Total members: <span <span style="font-weight: bold;" id="ajax_total"><?php echo $total;?></span>
                                    <br/> -->
                                    <div class="clr"></div>
                                </div>
                                <div class="binary_content">
                                    <ul id="tt">
                                        <img src="data:image/gif;base64," />
                                    </ul>
                                </div>
                                <?php echo $content_bottom; ?>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>


<?php echo $footer; ?>

<?php $js="$(document).ready(function(){

      $('#tt').tree({

        dnd: true,

        lines: true,

        animate: true,

        url: '" .$base. "index.php?route=account/personal/getJsonBinaryTree&id_user=".intval($idCustomer). "',

        onClick: function(node){
          $.ajax({
          url: '".$base. "index.php?route=account/personal/getInfoCustomer&id_user='+node.id,
          type: 'post',
          dataType: 'json',
          success: function(json) {
            $('#ajax_firstname').html(json['nameCustomer']);
            $('#ajax_telephone').html(json['telephone']);
            $('#ajax_floor').html(json['floor']);
            $('#ajax_total').html(json['total']);

          }
          })

        }
      });

    });"; ?>
<script type="text/javascript">
    <?php echo $js; ?>
</script>
